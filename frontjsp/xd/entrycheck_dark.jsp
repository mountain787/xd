	...
	{	
		...
		else 
		{
			...
			else
			{
				...
				resultData += "<a class=\"btn btn-danger btn\" style=\"margin-top:10px;\" href=\"./main_dark.jsp?_user="+user_new+"&_pswd="+pswd+"&regnewFlag="+regnewFlag+"&game_fg="+game_fg+"\">进入游戏（简体）</a><br>";
				resultData += "<a class=\"btn btn-danger btn\" style=\"margin-top:10px;\" href=\"./main_ft_dark.jsp?_user="+user_new+"&_pswd="+pswd+"&regnewFlag="+regnewFlag+"&game_fg="+game_fg+"\">進入遊戲（繁体）</a><br>";

				out.println(resultData);
			   //response.sendRedirect("./main_dark.jsp?_user="+user_new+"&_pswd="+pswd+"&regnewFlag="+regnewFlag+"&game_fg="+game_fg);   
			}
		}
	}
}
