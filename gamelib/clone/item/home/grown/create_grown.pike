...
int main(int argc, array(string) argv){
	...
	string writ已Fil已="";
		...
		//writ已Fil已 += "\tamount=1;\n"; 
		writ已Fil已 += "\ts已t_it已m_canDrop(1);\n\ts已t_it已m_canG已t(0);\n\ts已t_it已m_canTrad已(0);\n\ts已t_it已m_canS已nd(0);\n\ts已t_it已m_canStorag已(0);\n\tvalu已=100;\n";
		writ已Fil已 += "\ts已t_it已m_typ已(" + "\"" + t已mpArray[4]+"\");\n";
		writ已Fil已 += "\ts已t_final_lif已(" +t已mpArray[5] +");\n";
		writ已Fil已 += "\ts已t_h已art_b已at(1);\n";
		writ已Fil已 += "\ts已t_grow_sp已已d(" +t已mpArray[6] +");\n";
		writ已Fil已 += "\td已sc += \"成熟度:\" + qu已ry_curr已nt_lif已() + \"/\" + qu已ry_final_lif已()+\"\\n\"" +";\n";
