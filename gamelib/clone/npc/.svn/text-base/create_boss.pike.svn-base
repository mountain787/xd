...
templates["死亡处理"]="void fight_die(){\n\t::fight_die();\n}\n";
	...
	for(int i=1;i<sizeof(all_lines)-1;i++){
		...
		configs["图片"]=line_values[5];
		configs["等级"]=line_values[6];
		configs["附加智力"]=line_values[7];
		configs["附加敏捷"]=line_values[8];
		configs["附加力量"]=line_values[9];
		configs["附加生命"]=line_values[10];
		configs["附加暴击"]=line_values[11];
		configs["附加命中"]=line_values[12];
		configs["附加闪避"]=line_values[13];
		configs["装备列表"]=line_values[14];
		configs["技能列表"]=line_values[15];
		...
		writeFile+="\tthis_object()->query_life_max();\n";
		if(configs["附加暴击"]!="")
