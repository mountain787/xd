...
int main(string arg)
{
	...
	int esv_num = 0;
	werror("-----arg="+arg+"--\n");
	//--arg=2cubuchangku22000000000000000000000000000000030 0 sg=22 ss= eg=33 es=--
	//sscanf(arg,"%s %d %s %s %s %s",name,count,esv_str,sgd_str,ssv_str,egd_str);
	//sg=22 ss= eg=33 es=
	sscanf(arg,"%s %d %s %s %s %s",name,count,sgd_str,ssv_str,egd_str,esv_str);
	//--sgd_str=ss=#ssv_str=eg=33#egd_str=es=#esv_str=sg=22--
	werror("---sgd_str="+sgd_str+"#ssv_str="+ssv_str+"#egd_str="+egd_str+"#esv_str="+esv_str+"--\n");
	...
	if(env && ob){
		...
		//然后去玩家的输入作出严格的判断		
		//输入不能有负数
		//--sgd_num=0#ssv_num=0#egd_num=0#esv_num=0--
