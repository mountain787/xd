...
private protected mapping(int:array(string)) spec_item_list = ([]);
///////////////07/2/7

//记录所有白色装备的映射表
private protected mapping(int:array(string)) item_list = ([]);

//记录白色装备允许出现属性的映射表
private protected mapping(string:array(string)) item_attributes = ([]);

//用于生成物品文件后缀的映射表,现在暂时未用上
private protected mapping(string:int) postfix_map = ([
		"str_add"                    :0,
		"dex_add"                    :1,
		"think_add"                  :2,
		"all_add"					 :3,
...
//字母-数值映射表, 采用ascii码
private protected mapping(int:int) char_value = ([
		1		:49,
		2		:50,
		3		:51,
		4		:52,
...
private protected mapping(string:string) worlddrop_item_list = ([]);
