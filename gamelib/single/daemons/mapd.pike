...
private mapping(int:int) vip_fly_fee_config = ([
    0: 0,        // │     0      │  非会员   │  按等级计算   │
    1: 2000000,  // │     1      │  水晶会员 │  固定200万    │  (2万金)
    2: 1000000,  // │     2      │  黄金会员 │  固定100万    │  (1万金)
    3: 500000,   // │     3      │  白金会员 │  固定50万     │  (5000金)
    4: 500000,   // │     4      │  钻石会员 │  固定50万     │  (5000金)
]);
// └─────────────┴───────────┴──────────────┘
...
string get_all_kinds_map(){
	...
	foreach(block_list,string block){
		if(pinyin_to_cn[block]){
			...
			// 计算等级费用
			int level_fee;
			foreach(level_fly_fee_config; array range; int fee_value) {
